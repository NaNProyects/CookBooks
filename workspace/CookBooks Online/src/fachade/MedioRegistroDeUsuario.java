...
public class MedioRegistroDeUsuario extends MedioPanel {
	 ...
	 */
	public MedioRegistroDeUsuario(Interface inside2, MedioPanel ant) {
		this(inside2, Usuario.anonimo(), ant);
	}

	public MedioRegistroDeUsuario(Interface inside2, Usuario usuario,
			MedioPanel ant) {
		...
		labelErrores.setBounds(22, 384, 333, 165);
		add(labelErrores);
			...
			public void focusLost(FocusEvent e) {
				...
				try {
					...
					if (dniUsuario.getText().length() != 0)
						printError("El DNI no puede comenzar con 0 /n",
								errorDNI, true);
				}
				ValidadDNI();
			}
		...
		nombreUsuario.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (!(Character.isLetter(car) || Character.isWhitespace(car))
						|| nombreUsuario.getText().length() == 44) {
					if (!(e.isActionKey() || e.isControlDown()
							|| e.getKeyCode() == KeyEvent.VK_DELETE || e
							.getKeyCode() == KeyEvent.VK_BACK_SPACE)) {
						e.consume();
		...
		apellidoUsuario.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				Character car = e.getKeyChar();
				if (!(Character.isLetter(car) || Character.isWhitespace(car))
						|| apellidoUsuario.getText().length() == 44) {
					if (!(e.isActionKey() || e.isControlDown()
							|| e.getKeyCode() == KeyEvent.VK_DELETE || e
							.getKeyCode() == KeyEvent.VK_BACK_SPACE)) {
						e.consume();
		...
		targetaUsuario.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				Character car = e.getKeyChar();
				if (!(Character.isDigit(car))
						|| targetaUsuario.getText().length() == 18) {
					if (!(e.isActionKey() || e.isControlDown()
							|| e.getKeyCode() == KeyEvent.VK_DELETE || e
							.getKeyCode() == KeyEvent.VK_BACK_SPACE)) {
						e.consume();
		...
		telefonoUsuario.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				Character car = e.getKeyChar();
				if (!(Character.isDigit(car))
						|| telefonoUsuario.getText().length() == 13) {
					if (!(e.isActionKey() || e.isControlDown()
							|| e.getKeyCode() == KeyEvent.VK_DELETE || e
							.getKeyCode() == KeyEvent.VK_BACK_SPACE)) {
						e.consume();
		...
		Botonconfirmar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (ValidarUsuario()) {
					...
					user.setTelefono(telefonoUsuario.getText().toString());
		...
		cambiarPass = new JLabel("Cambiar Contrase\u00F1a");
		cambiarPass.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseReleased(MouseEvent e) { 
				inside.centro(new MedioCambioDeContraseña(inside, user, inside.center));
			}
		});
		...
		setFocusTraversalPolicy(new FocusTraversalOnArray(new Component[] {
				mailUsuario, passwordField, passwordField_1, dniUsuario,
				nombreUsuario, apellidoUsuario, direcUsuario, telefonoUsuario,
				targetaUsuario, pinTextField, Botonconfirmar, cancelar }));
		...
		EliminarCuenta.setBounds(52, 562, 144, 31);
		add(EliminarCuenta);
	...
	private boolean ValidarUsuario() {
		...
		return (((((user.getId().compareTo(0) != 0) || (ValidadDNI() & ValidadEmail()))
				& ValidadNombre()
				& ValidadApellido()
				& ValidadDireccion()
				& ValidadTelefono() & ValidadTargeta() & ValidadPin() & ValidadPass())));
	...
	private Boolean ValidarLongitudDNI() {
		if ((dniUsuario.getText().toString().length() < 9)
				&& (dniUsuario.getText().toString().length() > 6)) {
	...
	private Boolean ValidarExistenciaDNI() {
		try {
			if (!inside.contexto.existeDNI(dniUsuario.getText().toString())) {
				printError("El DNI pertenece a un usuario existente /n",
						errorDNI, false);
	...
	private Boolean ValidarExistenciaMail() {
		try {
			if (!inside.contexto.existeMail(mailUsuario.getText().toString())) {
				printError("El E-Mail pertenece a un usuario existente /n",
						errorMail, false);
				printError("Ocurrió un error /n", labelErrores, false);
				return true;
			} else {
	...
	private Boolean ValidadNombre() {
		if ((nombreUsuario.getText().length() < 45)
			...
			printError("El nombre debe tener entre 1 y 44 caracteres /n",
					errorNombre, false);
			return true;
	...
	private void validarEliminacion() {
		...
		panelConfirmacion.setBorder(new BevelBorder(BevelBorder.RAISED, null,
				null, null, null));
		...
		JLabel tituloFlotante = DefaultComponentFactory.getInstance()
				.createLabel("¿Seguro que desea eliminar su cuenta?");
		tituloFlotante.setFont(new Font("Tahoma", Font.BOLD, 20));
		tituloFlotante.setBounds(10, 11, 479, 25);
		panelConfirmacion.add(tituloFlotante);

		consinga = new JLabel(
				"Ingrese la contraseña para validar la eliminacion.");
		consinga.setBounds(20, 41, 389, 14);
		panelConfirmacion.add(consinga);

		lblpass = new JLabel("Contraseña:");
		lblpass.setBounds(30, 103, 79, 14);
		panelConfirmacion.add(lblpass);

		password = new JPasswordField();
		password.setBounds(119, 100, 196, 20);

		panelConfirmacion.add(password);
		password.setColumns(10);

		final JButton confirmar = new JButton("Confirmar");
		confirmar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (inside.contexto.chequearContraseña(user, new String(
						password.getPassword()))) {
					try {
						if (inside.contexto.eliminar(user)) {
							inside.top.setPanelLog(new Login(inside.top));
							inside.contexto.cerrarSesion();
							inside.left.permisos(inside.contexto
									.usuarioActual().getId());
							inside.center.refresh();
							printError(
									"El usuario posee pedidos, no se lo puede eliminar /n",
									labelErrores, false);
						} else {
							labelErrores.setForeground(Color.RED);
							printError(
					...
					printError("Contraseña incorrecta /n", labelErrores, false);
					panel.removeAll();
					panel.setVisible(false);
					swichBoton();
					...
					labelErrores.setForeground(Color.RED);
					printError("Contraseña incorrecta /n", labelErrores, true);
					printError(
							"El usuario posee pedidos, no se lo puede eliminar /n",
							labelErrores, false);
					printError("Ocurrio un error /n", labelErrores, false);
...
//			mailUsuario.setEnabled(!mailUsuario.isEnabled());
//			dniUsuario.setEnabled(!dniUsuario.isEnabled());
//			passwordField.setEnabled(!passwordField.isEnabled());
//			passwordField_1.setEnabled(!passwordField_1.isEnabled());
//		} 
//		nombreUsuario.setEnabled(!nombreUsuario.isEnabled());
//		apellidoUsuario.setEnabled(!apellidoUsuario.isEnabled());
//		direcUsuario.setEnabled(!direcUsuario.isEnabled());
//		telefonoUsuario.setEnabled(!telefonoUsuario.isEnabled());
//		targetaUsuario.setEnabled(!targetaUsuario.isEnabled());
//		pinTextField.setEnabled(!pinTextField.isEnabled());
		
		Botonconfirmar.setEnabled(!Botonconfirmar.isEnabled());
		cancelar.setEnabled(!cancelar.isEnabled());
		EliminarCuenta.setEnabled(!EliminarCuenta.isEnabled());
	}

	private void modoEdicion() {
		if (user.getId().compareTo(-1) != 0) {
			mailUsuario.setText(user.getEmail());
			dniUsuario.setText(user.getDni().toString());
			nombreUsuario.setText(user.getNombre());
			apellidoUsuario.setText(user.getApellido());
			direcUsuario.setText(user.getDireccion());
			telefonoUsuario.setText(user.getTelefono());
			targetaUsuario.setText(user.getTarjeta());
			pinTextField.setText(user.getPin());
			cambiarPass.setVisible(true);
			mailUsuario.setEnabled(false);
			dniUsuario.setEnabled(false);
			passwordField.setEnabled(false);
			passwordField_1.setEnabled(false);
			passwordField.setVisible(false);
			passwordField_1.setVisible(false);
			lblConfirmarContrasea.setVisible(false);
			contraseñaLabel.setVisible(false);
		} else {
