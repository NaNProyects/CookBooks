...
public class MedioEdicionDeLibro extends JPanel {
	...
	public MedioEdicionDeLibro(Interface inside2){
		this(inside2,
				new Libro(new Long(-1), "Titulo", null, "genero", "editorial",
	...
	public MedioEdicionDeLibro(Interface inside2,Libro libro2) {
		...
		JLabel lblTitulo = DefaultComponentFactory.getInstance().createTitle(tituloPanel);
		...
		autorLibro.setModel(new DefaultComboBoxModel(Contexto.ListaDeAutores("REVISAR").toArray()));
		...
		confirmar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				libro.setPrecio(((Double) precioLibro.getValue()).doubleValue());
				Contexto.AgragarLibro(libro);
				inside.centro(new MedioListaDeLibros(inside));
				}
			}
		});		
		
		confirmar.setIcon(new ImageIcon(MedioEdicionDeLibro.class.getResource("/fachade/Image/Clear Green Button.png")));
		...
		cancelar.setIcon(new ImageIcon(MedioEdicionDeLibro.class.getResource("/fachade/Image/Cancel Red Button.png")));
		...
		add(scrollVistaso);
		setFocusTraversalPolicy(new FocusTraversalOnArray(new Component[]{isbnLibro_1, tituloLibro, autorLibro, generoLibro, editorialLibro, idiomaLibro, precioLibro, confirmar, cancelar, scrollReseña, reseñaLibro, scrollVistaso, vistasoLibro}));
